...
//-------------------- ricoAjaxEngine.js
/*定义Rico.AjaxEngine类，作为一个同一向服务器申请数据的代理*/
Rico.AjaxEngine = Class.create();

Rico.AjaxEngine.prototype = {
   /*初始化*/
   initialize: function() {
      this.ajaxElements = new Array();/*html文档中的ajax请求对象(处理ajax回应的对象)*/
      this.ajaxObjects  = new Array();/*用户自定义的ajax请求对象(处理ajax回应的对象)*/
      this.requestURLS  = new Array();/*注册请求的url*/
   },
   /*给外界在代理里面注册html文档中的ajax请求对象*/
   registerAjaxElement: function( anId, anElement ) {
      if ( arguments.length == 1 )
         anElement = $(anId);
      this.ajaxElements[anId] = anElement;
   },
   /*给外界在代理里面注册用户自定义的ajax请求对象*/
   ...
   /*更新用户定义ajax请求对象，可以给用户自行定义ajaxUpdate处理，适应用户多样化的需求*/
