...
//MarkerBreakout轮询
public class MarketBreakoutPollingUtil {
	
	private static Logger logger = Logger.getLogger(MarketBreakoutPollingUtil.class);
	
	/**
	 * 判断是否达到MarketBreakout的执行条件，如果是则执行
	 */
	public static void marketBreakoutCheck(){
       ...
       /* for (int i = 0; i < MonitorPriceArray.length; i++) {
		...
		if(bean.getDirection().equals("0")){
			if(MonitorPriceArray[0].equals("B")){
				//System.out.println(RateUtil.getRateByCCy("W","USD"+"GBP"));
				//System.out.println(bean.getTranType()+"  "+bean.getWeCcy()+bean.getAnaCcy());
				//System.out.println(RateUtil.getRateByCCy(bean.getTranType(),bean.getWeCcy()+bean.getAnaCcy()));
