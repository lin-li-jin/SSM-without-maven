...
public class SubjectBo extends BaseBo{
    ...
    public void addnewExchange(SubjectItem item) {
        ExamContent examContent=new ExamContent();
        try {
            examContent.setExamContent(item.getExamContent().getBytes("utf-8"));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            logger.error("charset is throw error");
        }
        ExamAccType examAccType=new ExamAccType();
        examAccType.setAccTypeNo(item.getAccTypeNo());
        examContent.setExamAccTypeByAccTypeNo(examAccType);
        examContentDAO.save(examContent);
        List<ExamContent> examContents= null;
        try {
            examContents = examContentDAO.findByExample(examContent);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
        }
        if (examContents!=null && examContents.size()==1){
            item.setExamNo(examContents.get(0).getExamNo());
        }
    ...
    private void addOneClickExchange(SubjectItem item) {
        ...
        examOneClick.setUserType(ExamConst.TEACHER);
        examOneClick.setAccTypeNo(item.getAccTypeNo());
        examOneClick.setStep(ExamConst.DEFAULT_STEP);
